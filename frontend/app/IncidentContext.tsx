...
export function IncidentProvider({ children }: { children: React.ReactNode }) {
  ...
  const dataFreshness = state?.summary.data_freshness_minutes ?? state?.advisory.data_freshness_minutes ?? null;
  ...
  const value = useMemo(() => ({ currentAdvisory, currentEvent: state?.advisory.event_id ?? null, selectedAsset, lastUpdated, dataFreshness, activeResponseState, lastRefreshReason: lastReason, state, refreshing, error, setCurrentAdvisory, setSelectedAsset, refreshIncident }), [activeResponseState, currentAdvisory, dataFreshness, error, lastReason, lastUpdated, refreshIncident, refreshing, selectedAsset, state]);
